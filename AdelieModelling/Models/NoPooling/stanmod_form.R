...
parameters {
  ...
  vector<lower=0>[K] p_lam[I];          // Lambda for Poisson distributions on the number of dives
  
  vector[K] l_mu[I];                    // Mu for log-normal distributions on step length
  vector<lower=0>[K] l_sig[I];          // Sigma for log-normal distributions on step length
  
  vector<lower=0, upper=1>[K] w_rho[I]; // Rho for wrapped-Cauchy distributions on turning angle 
}
  ...
  for(i in 1:I){
      ...
      for (h in 1:K){
        logalpha[1,h] = log(delta[i,h]) +                                 // Initial distribution
                        poisson_lpmf(y1i[1] | p_lam[i,h] * y4i[1]) +      // Poisson on number of dives
                        lognormal_lpdf(y2i[1] | l_mu[i,h], l_sig[i,h]) +  // Log Normal on step length
                        wc_lpdf(y3i[1] | y3_0[i], w_rho[i,h]);            // Wrapped Cauchy on turning angle
      }
        ...
        for (j in 1:K) {
          for (l in 1:K) {
            accumulator[l] =  logalpha[t-1, l] +                                // Previous log-alpha value
                              ...
                              poisson_lpmf(y1i[t] | p_lam[i,j] * y4i[t]) +      // Poisson on number of dives
                              lognormal_lpdf(y2i[t] | l_mu[i,j], l_sig[i,j]) +  // Log Normal on step length
                              wc_lpdf(y3i[t] | y3i[t-1], w_rho[i,j]);           // Wrapped Cauchy on turning angle
          }
    ...
    p_lam[i,] ~ gamma(1,0.05);    // Lambda for Poisson distributions on the number of dives
    l_mu[i,] ~ normal(0,5);       // Mu for log-normal distributions on step length
    l_sig[i,] ~ student_t(3,0,1); // Sigma for log-normal distributions on step length
    w_rho[i,] ~ beta(1,1);        // Rho fro wrapped Cauchy on turning angle
