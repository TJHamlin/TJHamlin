...
init_fun <- function() {
  #Lambda for the Poisson distribution on the number of dives
  i_lam = abs(rnorm(3, c(0, 7, 20), 0.1))
  #
  #Mu for the log normal distributions on step length
  i_mu = rnorm(3, c(0, 1, 2), 0.1)
  #Sigma for the log normal distributions on step length
  i_sig = abs(rnorm(3, c(0.5, 0.5, 0.5), 0.1))
  #
  #Rho for the wrapped Cauchy distributions on turning angle
  i_rho = rbeta(3, 2, 5)
  #
  #Initial probability distribution
  i_delta = c(rdirch(1,sts))
  #Transition probability matrix
  i_G <- rdirch(sts,sts)
  #
  list(p_lam = i_lam, l_mu = i_mu, l_sig = i_sig, w_rho=i_rho, delta = i_delta, G = i_G)
} 
