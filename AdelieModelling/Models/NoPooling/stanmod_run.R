...
init_fun <- function() {
  #Lambda for the Poisson distributions on number of dives
  i_lam = matrix(rep(abs(rnorm(sts, c(0, 5, 20), 0.5)),2), nrow=inds, ncol=sts, byrow=T, dimnames=NULL)
  #
  #Mu for the log normal distributions on step length
  i_mu = matrix(rep(rnorm(sts, c(0, 1, 2), 0.1),2), nrow=inds, ncol=sts, byrow=T, dimnames=NULL)
  #Sigma for the log normal distributions on step length
  i_sig = matrix(rep(abs(rnorm(sts, c(0.5, 0.5, 0.5), 0.1)),2), nrow=inds, ncol=sts, byrow=T, dimnames=NULL)
  #
  #Rho for the wrapped Cauchy distributions on turning angle
  i_rho = matrix(rep(rbeta(sts, 2, 5),2), nrow=inds, ncol=sts, byrow=T, dimnames=NULL)
  #
  ...
  #
  list(p_lam = i_lam, l_mu = i_mu, l_sig = i_sig, delta = i_delta, G = i_G)
} 
...
mod_stan = stan(file = "mod.stan",
                ...
                warmup = itrs/5,
                cores = 4,
                pars=c("lp__", "G", "p_lam", "l_mu", "l_sig", "w_rho"),
                save_warmup=F
)
