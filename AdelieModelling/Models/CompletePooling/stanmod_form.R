...
parameters {
  simplex[K] delta;                   // The initial distribution
  simplex[K] G[K];                    // A K x K matrix that holds the transition probabilities
  
  vector<lower=0>[K] p_lam;           // Lambda for Poisson distributions on the number of dives
  
  vector[K] l_mu;                     // Mu for log-normal distributions on step length
  vector<lower=0>[K] l_sig;           // Sigma for log-normal distributions on step length
  
  vector<lower=0, upper=1>[K] w_rho;  // Rho for wrapped-Cauchy distributions on turning angle  
}
...
model {
  ...
  for(i in 1:I){
      ...
      for (h in 1:K){
        logalpha[1,h] = log(delta[h]) +                               // Initial distribution
                        poisson_lpmf(y1i[1] | p_lam[h] * y4i[1]) +    // Poisson on the number of dives
                        lognormal_lpdf(y2i[1] | l_mu[h], l_sig[h]) +  // Log Normal on the step lengths
                        wc_lpdf(y3i[1] | y3_0[i], w_rho[h]);          // Wrapped Cauchy on the turning angles
      }
      ...
      // At each observation we consider every state-to-state ("j" being the current state and "l" being the previous) transition possible. At each current state "j" we calculate a precursor log-alpha value (denoted as accumulator) given each possible previous state "l". This can be considered a probability of transitioning from state "i" to state "j" This value is based on the following summation of the previous log-alpha value for that state, the transition probability from "l" to "j", and the log pdf/pmf of the state-dependent distributions on our data.
        ...
        for (j in 1:K) {
          for (l in 1:K) {
            accumulator[l] =  logalpha[t-1, l] +                            // Previous log-alpha value
                              log(G[l, j]) +                                // Transition probability from state i to j
                              poisson_lpmf(y1i[t] | p_lam[j] * y4i[t]) +    // Poisson on number of dives
                              lognormal_lpdf(y2i[t] | l_mu[j], l_sig[j]) +  // Log Normal on step length
                              wc_lpdf(y3i[t] | y3i[t-1], w_rho[j]);         // Wrapped Cauchy on turning angle
          }
  ...
  // Establish the priors for our state-dependent parameters.
  
  p_lam ~ gamma(1,0.05);    // Lambda for Poisson distributions on the number of dives
  l_mu ~ normal(0,5);       // Mu for log normal distributions on the step lengths
  l_sig ~ student_t(3,0,1); // Sigma for log normal distributions on the step lengths
  w_rho ~ beta(1,1);        // Rho for wrapped Cauchy distributions on the turning angles
}
'
